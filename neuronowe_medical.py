...
print(f"Rozkład klas: {df['target'].value_counts().to_dict()}")

# Zmienne numeryczne z Heart Disease
...
# 2. EKSPERYMENTY – WPŁYW PARAMETRÓW SIECI
results = []
...
# 4. NAJLEPSZY MODEL – SZCZEGÓŁOWY RAPORT
...
# 7. METODY NIENADZOROWANE
# Dane BEZ kolumny target
X_clust = StandardScaler().fit_transform(df[features].values)

# KMeans
kmeans = KMeans(n_clusters=2, random_state=42, n_init=10)
labels = kmeans.fit_predict(X_clust)

ari = adjusted_rand_score(df['target'].values, labels)
print(f"Adjusted Rand Index: {ari:.4f}")  # 1.0 idealne, 0.0 losowe

# Wizualizacja przez PCA
pca = PCA(n_components=2)
X_2d = pca.fit_transform(X_clust)

plt.figure(figsize=(8, 5))
plt.scatter(X_2d[:, 0], X_2d[:, 1], c=labels, cmap='coolwarm', alpha=0.6)
plt.title('KMeans – wizualizacja klastrów (PCA)')
plt.xlabel('PC1')
plt.ylabel('PC2')
plt.savefig('klastry.png', dpi=150)
print("Klastry: klastry.png")